/**
 * Image editor component for replacing and editing slide images.
 *
 * This component provides a comprehensive image editing interface with three
 * main tabs: AI generation, upload, and edit. It allows users to generate
 * images using AI prompts, upload custom images, or edit existing images
 * with focus point and object-fit controls.
 *
 * Features:
 * - AI image generation from text prompts
 * - File upload with validation (5MB limit, image types only)
 * - Previous generated images gallery
 * - Uploaded images gallery with delete functionality
 * - Focus point adjustment (click to set crop focus)
 * - Object-fit controls (cover, contain, fill)
 * - Real-time preview updates
 * - Smooth sheet animations
 *
 * The component integrates with PresentationGenerationApi for AI generation
 * and ImagesApi for upload/delete operations. It tracks analytics events
...
import type { SlideProperties, ImageProperties } from "@/types/slide";

/**
 * Props for the ImageEditor component.
 *
 * @property initialImage - Current image URL to display and edit, or null.
 * @property imageIdx - Zero-based index of the image in the slide (for
 *   properties lookup). Defaults to 0.
 * @property slideIndex - Zero-based index of the slide containing the image.
 * @property className - Additional CSS classes for the editor container.
 * @property promptContent - Original prompt used to generate the image (if any).
 *   Displayed in the editor for reference.
 * @property properties - Optional slide properties containing image-specific
 *   settings like focus point and object-fit.
 * @property onClose - Optional callback invoked when editor is closed.
 *   Called after animation completes (300ms delay).
 * @property onImageChange - Optional callback invoked when a new image is selected.
 *   Receives the new image URL and optional prompt string.
 * @property onFocusPointClick - Optional callback invoked when focus point or
 *   object-fit is changed. Receives ImageProperties object with new settings.
 */
interface ImageEditorProps {
  ...
  onFocusPointClick?: (propertiesData: ImageProperties) => void;
}

/**
 * Image editor component.
 *
 * Renders a side sheet panel with tabs for generating, uploading, and editing
 * images. Handles image selection, focus point adjustment, and property updates.
 *
 * @param props - Component props containing image data, slide info, and callbacks.
 * @returns JSX element containing the image editor sheet panel.
 */
const ImageEditor = ({
  initialImage,
  imageIdx = 0,
  ...
  // State management for image preview and generation
  ...
  // Focus point determines which part of the image is visible when using object-fit: cover
  ...
  // Object-fit determines how image fills its container
  const [objectFit, setObjectFit] = useState<"cover" | "contain" | "fill">(() => {
    const fit = properties?.[imageIdx]?.initialObjectFit;
  ...
  // Ref to preview image element for focus point calculations
  const imageRef = useRef<HTMLImageElement>(null);

  /**
   * Effect: Sync preview image with initialImage prop.
   *
   * Updates preview when initialImage changes externally.
   */
  useEffect(() => {
  ...
  /**
   * Effect: Load previous generated images on mount.
   *
   * Fetches previously generated images when editor opens, but only if
   * not already loaded. Used to show user's generation history.
   */
  useEffect(() => {
   ...
   * onClose callback after 300ms to allow animation to complete.
   */
  const handleClose = () => {
  ...
  /**
   * Fetches previously generated images from the API.
   *
   * Loads user's generation history to display in the "Previous Generated"
   * section. Tracks analytics event and handles errors with toast notifications.
   *
   ...
   *
   * When user selects an image (from generation, upload, or previous images),
   * updates the preview and calls onImageChange callback with the new URL
   * and original prompt content.
   *
   * @param newImage - URL string of the selected image.
   */
   ...
   *
   * When in focus point mode, calculates click position relative to image
   * and converts to percentage coordinates (0-100). Updates focus point
   * state and applies it immediately to the image for real-time feedback.
   *
   * Focus point determines which part of the image is visible when using
   * object-fit: cover. Useful for cropping or emphasizing specific areas.
   *
   * @param e - Mouse click event on the image.
   */
  const handleFocusPointClick = (e: React.MouseEvent) => {
    if (!isFocusPointMode || !imageRef.current) return;

    const rect = imageRef.current.getBoundingClientRect();
    // Convert click coordinates to percentage (0-100)
    const x = Math.max(
      0,
    ...
    // Apply the focus point in real-time for immediate visual feedback
    if (imageRef.current) {
   ...
   *
   * When exiting focus point mode, saves current properties. When entering,
   * enables click-to-set-focus-point functionality.
   */
  ...
  /**
   * Handles object-fit property change.
   *
   * Updates object-fit CSS property which controls how the image fills
   * its container:
   * - cover: Image covers entire container, may be cropped
   * - contain: Image fits entirely within container, may have empty space
   * - fill: Image stretches to fill container, may be distorted
   *
   * Applies change immediately to preview and saves to properties.
   *
   * @param fit - Object-fit value to apply.
   */
  ...
  /**
   * Saves image properties (focus point and object-fit) to parent.
   *
   * Packages current focus point and object-fit settings into ImageProperties
   * object and calls onFocusPointClick callback. This persists the settings
   * for the image in the slide data.
   *
   * @param fit - Current object-fit value.
   * @param focusPoint - Current focus point coordinates (x, y percentages).
   */
  ...
  /**
   * Generates new images using AI from text prompt.
   *
   * Calls PresentationGenerationApi to generate an image based on the
   * user's text prompt. Validates prompt is not empty, tracks analytics,
   * and handles errors with user-friendly messages. Updates preview
   * with generated image URL.
   *
   * @throws Displays error toast if generation fails or prompt is empty.
   */
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      setPreviewImages(response as string | null);
    } catch (err) {
   ...
   *
   * Validates file size (5MB limit) and type (image files only), then uploads
   * to the server via ImagesApi. Updates preview with uploaded image URL.
   * Tracks analytics event and handles errors with toast notifications.
   *
   * @param event - File input change event containing the selected file.
   * @throws Displays error toast if validation fails or upload fails.
   */
  ...
  /**
   * Fetches user's uploaded images from the server.
   *
   * Loads all images previously uploaded by the user to display in the
   * upload tab gallery. Handles errors with toast notifications.
   *
   * @throws Displays error toast if API call fails.
   */
   ...
   *
   * When upload tab is selected, automatically loads uploaded images
   * to display in the gallery. This lazy-loads images only when needed.
   *
   * @param value - Tab value that was selected.
   */
  ...
  /**
   * Handles deletion of an uploaded image.
   *
   * Calls ImagesApi to delete the image from the server and removes it
   * from the local uploadedImages state. Shows success/error toasts.
   *
   * @param image_id - Unique identifier of the image to delete.
   * @throws Displays error toast if deletion fails.
   */
